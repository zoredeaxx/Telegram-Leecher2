...
@colab_bot.on_message(filters.command("tgupload") & filters.private)
...
@colab_bot.on_message(filters.command("dirupload") & filters.private)
...
async def handle_url(client, message):
    ...
    if not BOT.State.task_going and BOT.State.started:
        ...
        for _ in range(3):
            if temp_source and temp_source[-1].startswith("["):
                BOT.Options.custom_name = temp_source.pop()[1:-1]
            elif temp_source and temp_source[-1].startswith("{"):
                BOT.Options.zip_pswd = temp_source.pop()[1:-1]
            elif temp_source and temp_source[-1].startswith("("):
                BOT.Options.unzip_pswd = temp_source.pop()[1:-1]
            else:
                break
...
async def handle_options(client, callback_query):
    ...
    if data in ["normal", "zip", "unzip", "undzip"]:
        ...
        if message.reply_to_message_id:
            await colab_bot.delete_messages(chat_id=message.chat.id, message_ids=message.reply_to_message_id)
        
        MSG.status_msg = await colab_bot.send_message(OWNER, "#STARTING_TASK\n\n**Starting your task in a few Seconds...🦐**",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel ❌", callback_data="cancel")]]))
        MSG.sent_msg = await colab_bot.send_message(DUMP_ID, "`Initializing Task...`")
        ...
        await message.edit_text("Send a Text to Set as PREFIX by REPLYING THIS MESSAGE »")
        BOT.State.prefix = True

    elif data == "set-suffix":
        await message.edit_text("Send a Text to Set as SUFFIX by REPLYING THIS MESSAGE »")
