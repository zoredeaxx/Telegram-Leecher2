...
async def taskScheduler(bot_client):
    ...
    try:
        ...
        logging.error(f"A critical error occurred in taskScheduler: {e}", exc_info=True)
        TaskError.state = True
        TaskError.text = f"Task failed critically: {e}"
    
    finally:
        logging.info("Task has ended. Sending final logs.")
        is_leech_task = BOT.Mode.mode != "mirror"
        await SendLogs(bot_client, is_leech_task)
    ...
    if is_dir:
        for s in source:
            if not ospath.exists(s):
                ...
                await cancelTask(bot_client, "Provided directory does not exist!")
                return
            Paths.down_path = s
            if is_zip:
