...
async def handle_options(client, callback_query):
    ...
    if callback_query.data in ["normal", "zip", "unzip", "undzip"]:
    ...
    elif callback_query.data in ["code-Monospace", "p-Regular", "b-Bold", "i-Italic", "u-Underlined"]:
        ...
        if callback_query.data in ["convert-true", "convert-false"]:
            BOT.Options.convert_video = (True if callback_query.data == "convert-true" else False)
            BOT.Setting.convert_video = ("Yes" if callback_query.data == "convert-true" else "No")
        ...
        BOT.Setting.stream_upload = ("Media" if callback_query.data == "media" else "Document")
        await send_settings(client, callback_query.message, callback_query.message.id, False)
